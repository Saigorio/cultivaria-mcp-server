...
const UpdateSetpointSchema = zod_1.z.object({
    metric: zod_1.z.enum(['ec', 'temp_agua', 'temp_aire', 'humedad_aire', 'vpd', 'ph']),
...
const SetActuatorModeSchema = zod_1.z.object({
    device: zod_1.z.enum(['extractor_general', 'ventilador_recirculacion']),
...
const SetClimateRemotePowerSchema = zod_1.z.object({
    ...
    mode: zod_1.z.enum(['cool', 'heat', 'dry', 'fan']).optional().default('cool'),
    ...
    server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
        ...
        try {
            ...
            if (name === 'trigger_irrigation') {
                ...
                const updateRes = await (0, firebaseClient_js_1.updateModuleData)('commands/irrigation', {
                    status: statusText,
            ...
            if (name === 'set_actuator_mode') {
                ...
                const patchPayload = { [device]: mode };
                if (on_minutes !== undefined)
                    patchPayload[`${device}_on_min`] = on_minutes;
                if (off_minutes !== undefined)
                    patchPayload[`${device}_off_min`] = off_minutes;
            ...
            if (name === 'update_strategy') {
                ...
                const updateRes = await (0, firebaseClient_js_1.updateModuleData)('strategy', {
            ...
            if (name === 'set_climate_remote_power') {
                ...
                const updateRes = await (0, firebaseClient_js_1.updateModuleData)('virtual_ac', {
                    power,
                    temp,
                    mode,
                    turbo,
            ...
            if (name === 'set_climate_remote_swing') {
                ...
                if (vertical_swing !== undefined)
                    patchData.verticalSwing = vertical_swing;
                if (horizontal_swing !== undefined)
                    patchData.horizontalSwing = horizontal_swing;
                const updateRes = await (0, firebaseClient_js_1.updateModuleData)('virtual_ac', patchData, module_id);
                const responsePayload = {
                    success: updateRes.success,
